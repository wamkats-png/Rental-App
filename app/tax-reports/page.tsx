...
export default function TaxReportsPage() {
  const { landlord, properties, units, payments, maintenance } = useApp();
  ...
  const landlordExpenses = periodMaintenance.filter(m => m.payer === 'Landlord').reduce((s, m) => s + m.cost, 0);
  ...
  if (isIndividual) {
    ...
    taxLiability = grossIncome > INDIVIDUAL_THRESHOLD ? Math.round((grossIncome - INDIVIDUAL_THRESHOLD) * 0.12) : 0;
  } else {
    netTaxableIncome = Math.max(grossIncome - landlordExpenses, 0);
    taxLiability = Math.round(netTaxableIncome * 0.30);
  }
  ...
  const monthlyData = Array.from({ length: 12 }, (_, i) => {
    ...
    const monthName = new Date(year, monthIndex).toLocaleDateString('en-US', { month: 'short', year: 'numeric' });
  ...
  const handleExport = () => {
    const data = [{
      ...
      Allowable_Expenses: isIndividual ? 'N/A' : landlordExpenses,
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={handleExport} className="bg-gray-100 text-gray-700 px-4 py-2.5 rounded-lg hover:bg-gray-200 transition font-medium text-sm">Export Report</button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <div className="space-y-3 text-sm">
          ...
          {isIndividual ? (
            ...
            <>
              <div className="flex justify-between py-2 border-b"><span className="text-gray-600">Less: Allowable Expenses (Landlord Maintenance)</span><span className="font-medium text-orange-600">- {formatUGX(landlordExpenses)}</span></div>
